...
void drawCoffeeMachineStyled(float x, float y, float scale) {
    ...
    drawCircleFilled(x + 20 * scale, y + 125 * scale, 5 * scale, 50);
    drawCircleFilled(x + 95 * scale, y + 125 * scale, 5 * scale, 50);
}

// Brown coffee grinder
void drawBrownCoffeeMakerStyled(float x, float y, float scale) {
    ...
    // Hopper (top funnel) - wider and more visible
    setColor(darkBrown);
    glBegin(GL_QUADS);
...
void drawSteamSquares(float cx, float cy, float size, int count) {
    ...
    for (int i = 0; i < count; i++)
        drawFilledRect(cx - size / 2, cy + i * (size + 4), size, size, white, outline, 1.0f);
}
...
void drawCatPixel(float cx, float cy, float scale, int colorIndex) {
    ...
    setColor(body); drawEllipse(cx, cy, 32 * scale, 28 * scale);
    setColor(lighter); drawEllipse(cx - 5 * scale, cy - 8 * scale, 18 * scale, 20 * scale);
    setColor(body); drawCircleFilled(cx - 22 * scale, cy + 22 * scale, 20 * scale);
    setColor(lighter); drawEllipse(cx - 22 * scale, cy + 18 * scale, 14 * scale, 12 * scale);
    ...
    glLineWidth(1.0f); setColor({ 0.3f, 0.3f, 0.3f });
    glBegin(GL_LINES);
    ...
    setColor(darker); drawCircleFilled(cx + 42 * scale, cy - 26 * scale, 6 * scale);
}
...
void drawSleepingCatPixel(float x, float y, float scale) {
    ...
    setColor(body); drawEllipse(x, y, 28 * scale, 18 * scale);
    drawCircleFilled(x + 20 * scale, y + 4 * scale, 16 * scale);
    setColor(lighter); drawEllipse(x + 28 * scale, y + 6 * scale, 10 * scale, 7 * scale);
    ...
    glLineWidth(2.0f); setColor({ 0.2f, 0.2f, 0.2f });
    glBegin(GL_LINES);
...
void drawPlantPixel(float cx, float cy, int colorIndex) {
    ...
    glBegin(GL_TRIANGLES);
    glVertex2f(cx - 30, cy - 35); glVertex2f(cx - 20, cy - 35); glVertex2f(cx - 25, cy - 10);
    glEnd();
    ...
    glLineWidth(3.0f); setColor({ 0.2f, 0.5f, 0.2f });
    glBegin(GL_LINES);
    ...
    for (auto& leaf : leaves) {
        ...
        glEnd();
        glLineWidth(1.5f); setColor(leafDark);
        glBegin(GL_LINES);
        glVertex2f(0, 0); glVertex2f(0, 25 * leaf.size);
        glVertex2f(0, 15 * leaf.size); glVertex2f(-15 * leaf.size, 20 * leaf.size);
        glVertex2f(0, 15 * leaf.size); glVertex2f(15 * leaf.size, 20 * leaf.size);
        glEnd();
        setColor(leafLight); drawEllipse(-8 * leaf.size, 12 * leaf.size, 6 * leaf.size, 8 * leaf.size);
        glPopMatrix();
    }
}
...
void drawWindowPixel(float x, float y, float w, float h, float openAmount) {
    ...
    // Draw tree through window
    drawTreeThroughWindow(x + w / 2, y + 20, 1.2f);

    glLineWidth(3.0f); setColor({ 0.73f, 0.56f, 0.36f });
    glBegin(GL_LINES);
...
void drawShelvesWithItems(float x, float y) {
    ...
    // Items on top shelf - jars and containers
    ...
    // Items on bottom shelf - cups and bowls
...
void drawCoffeeMenuSign(float x, float y) {
    ...
    // "COFFEE MENU" text representation (lines)
    setColor({ 0.3f, 0.3f, 0.3f });
    glLineWidth(2.0f);
    // Title area
    glBegin(GL_LINES);
...
void drawChalkboardMenu(float x, float y, float w, float h) {
    ...
    // Menu items (simple lines representing text)
    float itemY = y + h - 55;
    for (int i = 0; i < 4; i++) {
        glBegin(GL_LINES);
...
void drawBookshelf(float x, float y) {
    ...
    for (int shelf = 0; shelf < 4; shelf++) {
        ...
        for (int book = 0; book < 6; book++) {
            ...
            drawFilledRect(bx + 2, y + 35 + shelf * 70, 3, bookHeight - 10, { std::min(1.0f, bc.r * 1.3f), std::min(1.0f, bc.g * 1.3f), std::min(1.0f, bc.b * 1.3f) }, { std::min(1.0f, bc.r * 1.3f), std::min(1.0f, bc.g * 1.3f), std::min(1.0f, bc.b * 1.3f) }, 0);
...
void drawEntryArea(float x, float y) {
    ...
    // Door positioned on the left wall
    float doorX = x;
    ...
    // Bulletin board on wall (moved to better position)
...
void drawScenePixels() {
    ...
    setColor(wall);
    glBegin(GL_QUADS);
    glVertex2f(0, 0); glVertex2f(SCENE_WIDTH, 0);
    glVertex2f(SCENE_WIDTH, windowHeight); glVertex2f(0, windowHeight);
    glEnd();

    drawFilledRect(0, windowHeight - 24, SCENE_WIDTH, 24, ceiling, edge, 2.0f);

    glLineWidth(2.0f); setColor(edge);
    glBegin(GL_LINES);
    glVertex2f(0, windowHeight * 0.32f); glVertex2f(SCENE_WIDTH, windowHeight * 0.32f);
    glEnd();
    ...
    // Brown coffee grinder - adjusted position to not overlap with shelves
    ...
    setColor({ 0.8f, 0.3f, 0.4f }); drawCircleFilled(SCENE_WIDTH - 190, windowHeight * 0.62f, 40);
    setColor({ 0.3f, 0.6f, 0.8f }); drawCircleFilled(SCENE_WIDTH - 210, windowHeight * 0.58f, 30);
    setColor({ 0.9f, 0.7f, 0.2f }); drawCircleFilled(SCENE_WIDTH - 170, windowHeight * 0.6f, 25);
}
...
void spawnSmoke(float x, float y) {
    Smoke s; s.x = x; s.y = y; s.life = 1.0f;
...
void keyboard(unsigned char key, int x, int y) {
    ...
    case 'a': case 'A': cameraX -= CAMERA_STEP; break;
    case 'd': case 'D': cameraX += CAMERA_STEP; break;
    }
    glutPostRedisplay();
}
...
void specialKeys(int key, int x, int y) {
    switch (key) {
    case GLUT_KEY_LEFT: cameraX -= CAMERA_STEP; break;
    case GLUT_KEY_RIGHT: cameraX += CAMERA_STEP; break;
    case GLUT_KEY_UP:
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Cat Cafe Scene");
